...
CREATE INDEX IF NOT EXISTS idx_compressed_profiles_subject
    ON compressed_profiles (database_id, subject_id, status);
...
CREATE INDEX IF NOT EXISTS idx_agentic_files_path
    ON agentic_memory_files (database_id, path);
CREATE INDEX IF NOT EXISTS idx_agentic_files_search
    ON agentic_memory_files USING gin (search_text);
...
DROP TRIGGER IF EXISTS trg_agentic_files_search_text ON agentic_memory_files;
